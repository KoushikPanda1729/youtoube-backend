...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  if (!(req?.files && Array.isArray(req?.files?.thumbnail))) {
    throw new ApiError(400, "Thambnail file is required");
  }
...
const getVideoById = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponce(200, video[0], "video details fetched successfully"));
});
...
const updateVideo = asyncHandler(async (req, res) => {
  ...
  console.log("old😊😊>>", thumbnailToDelete);
...
const deleteVideo = asyncHandler(async (req, res) => {
  ...
  await deleteFromCloudinary(video.thumbnail); // video model has thumbnail public_id stored in it->check videoModel
  await deleteFromCloudinary(video.videoFile, "video"); // specify video while deleting video
